...
var sec=String("0"+d.getUTCSeconds()).slice(-2);
  if (oldUserChannel === undefined) {
  	//user joined channel
  	vlChannel.send('`'+year+'-'+month+'-'+day+' ['+hour+':'+min+':'+sec+'] `  '+newMember.displayName+'-'+oldMember.user.username+' ***JOINED*** _'+newUserChannel+'_');
  }
  else {
		if (newUserChannel === undefined) {
			//user left channel
			vlChannel.send('`'+year+'-'+month+'-'+day+' ['+hour+':'+min+':'+sec+'] `  '+oldMember.displayName+'-'+oldMember.user.username+' ***LEFT*** _'+oldUserChannel+'_');
			}
		else {
			//user switched channel
			if (newUserChannel!=oldUserChannel) {
				vlChannel.send('`'+year+'-'+month+'-'+day+' ['+hour+':'+min+':'+sec+'] `  '+oldMember.displayName+'-'+oldMember.user.username+' ***SWITCHED*** _'+oldUserChannel+'_ to _'+newUserChannel+'_');
				}
			}
  }		
});
